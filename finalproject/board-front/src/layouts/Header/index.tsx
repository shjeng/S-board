...
const Header = () => {
  ...
  const MyPageButton = () =>{
    ...
    //      event handler: 마이페이지 버튼 클릭 이번트 처리 함수          //
    ...
    //      event handler: 로그인 버튼 클릭 이번트 처리 함수          //
  ...
  const UploadButton = () =>{
    ...
    //    function: patchBoardResponse 처리 함수 (게시물 수정)   // 
    ...
    const onUploadButtonClickHandler = async () =>{
      ...
      for(const file of boardImageFileList){
        ...
        const url = await fileUploadRequest(data); // 백엔드 쪽에서 반환을 url만 반환해줌 
        if(url) boardImageList.push(url);
      }
      // FormData는 HTML form 태그의 데이터를 쉽게 구성하고 전송할 수 있는 JavaScript 객체입니다. 
      //주로 파일 업로드와 함께 사용되며, append 메서드를 사용하여 키-값 쌍을 추가할 수 있습니다.
    ...
    <div id='header'>
      <div className='header-container'>
        <div className='header-left-box' onClick={onLogoClickHandler}>
          ...
          <div className='header-logo'>{'S Board'}</div>
        </div>
        <div className='header-right-box'>
          {(isMainPage||isUserPage || isBoardWritePage || isBoardUpdatePage || isSearchPage) && <SearchButton /> }
