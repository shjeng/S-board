...
const Authentication = () => {
      ...
      <div className='auth-card'>
        <div className='auth-card-box'>
          <div className='auth-card-top'>
            ...
            <InputBox ref={passwordRef} label='패스워드' type={passwordType} placeholder='비밀번호를 입력해주세요.' error={error} value={password} onChange={onPasswordChangeHandler} icon={passwordButtonIcon} 
  ...
  const SignUpCard = () => {
    ...
    const [isAgreedPersonalError, setAgreedPersnalError] = useState<boolean>(false);
    ...
    const onAgreedPersonalClickHandler = () =>{
      ...
      setAgreedPersnalError(false);

    }
    // event handler: 다음 버튼 클릭 이벤트 처리      //
    const onNextButtonClickHandler = () =>{
      const emailPattern = /^[a-zA-Z0-9]*@([-.]?[a-zA-Z0-9])*\.[a-zA-Z]{2,4}$/;
      const isEmailParttern = emailPattern.test(email);
      if(!isEmailParttern){
        setEmailError(true);
        setEmailErrorMessage('이메일 주소 포멧이 맞지 않습니다.');
      }
      const passwordPattern = /^\S+$/; 
      const isPasswordParttern = passwordPattern.test(password);
      if(!isPasswordParttern){
        setPasswordError(true);
        setPasswordErrorMessage('비밀번호에 공백이 포함되어 있습니다.');
      }
      ...
      if(!isCheckedPassword || !isPasswordParttern || !isEqualPassword) {
        passwordRef.current?.focus();
        return;
      };
      setPage(2);
    ...
    const onPasswordCheckDownHandler = (event: KeyboardEvent<HTMLInputElement>) => {
      if(event.key !== 'Enter') return;
        ...
        <div className='auth-card-box'>
          <div className='auth-card-top'>
            ...
            {page === 1 &&(
              ...
              <InputBox ref={passwordCheckRef} label='비밀번호 확인*' type={passwordCheckType} placeholder='비밀번호를 다시 입력해주세요.' value={passwordCheck} onChange={onPasswordCheckChangeHandler} error={isPasswordCheckError} message={passwordCheckErrorMessage} icon={passwordCheckButtonIcon} onButtonClick={onPasswordCheckButtonClickHandler} onKeyDown={onPasswordCheckDownHandler}/>
              </>
            )}
            {page===2 && (
              <>
              ...
              <InputBox ref={telNumberRef} label='핸대폰 번호*' type='text' placeholder='핸드폰 번호를 입력해주세요.' value={telNumber} onChange={onTelNumberChangeHandler} error={isTelNumberError} message={telNumberErrorMessage} onKeyDown={onTelNumberKeyDownHandler}/>
              <InputBox ref={addressRef} label='주소*' type='text' placeholder='우편번호 찾기' value={address} onChange={onAddressChangeHandler} error={isAddressError} message={addressErrorMessage} icon='expand-right-light-icon' onButtonClick={onAddressButtonclickHandler} onKeyDown={onAddressKeyDownHandler} readonly={true}/>
        ...
        <div className='auth-jumbotron-box'>
          <div className='auth-jumbotron-contents'>
            ...
            <div className='auth-jumbotron-text-box'>
              <div className='auth-jumbotron-text'>{'반가워요!'}</div>
              <div className='auth-jumbotron-text'>{'S BOARD 입니다.'}</div>
            </div>
          </div>
        </div>
